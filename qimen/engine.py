...
# 第三者偵測用神（EP03）
THIRD_PARTY_GAN = {"丙": "男性第三者", "丁": "女性第三者"}

# 符號含義表（EP03）
SYMBOL_MEANINGS = {
    "開門": "開朗直率、思想開放", "杜門": "封閉隱藏、高科技行業", "驚門": "爭執吵鬧、擔心憂慮",
    "死門": "固執不開心、死胡同、前無去路", "景門": "文書景象", "傷門": "傷害破損",
    "休門": "休養和諧", "生門": "生機財利、做生意、經濟條件好",
    "白虎": "兇猛脾氣大、做事果斷決絕", "太陰": "隱秘暗地裡",
    "值符": "高端有地位、管理層人員", "六合": "合作合夥",
    "天芮星": "問題隱患", "天心星": "有心計善策劃、做事果斷決絕、能力強", "天任星": "有責任心",
    "天蓬星": "暗昧智謀、膽大妄為、重視肉慾", "天英星": "文明血光、著重外貌形象",
    "天沖星": "行動衝動、容易衝動、性格急", "天輔星": "文教溫和、學歷高",
    "天柱星": "驚恐破敗",
}

# EP04 擴展符號含義 — 基於案例實際解讀
SYMBOL_MEANINGS_EP04 = {
    '生門': '生機財利、做生意、經濟條件好',
    '杜門': '封閉隱藏、高科技行業',
    '開門': '開朗直率、思想開放',
    '死門': '固執不開心、死胡同、前無去路',
...
# EP05 扩展符号含义 — 事业案例
SYMBOL_MEANINGS_EP05 = {
    '驚門': '擔心憂慮、整日擔心、每日憂心忡忡',
    '傷門': '傷害破損、衝突爭執',
    '天輔星': '文教溫和、學歷高',
    '天心星': '有心計善策劃、做事果斷決絕、能力強',
    '值符': '高端有地位、管理層人員',
    '白虎': '兇猛脾氣大、做事果斷決絕',
    '九天': '到處走動、規模大',
}

# 天干下臨解讀（EP05）— 天盤干 + 地盤干 組合含義
TIANGAN_LIN_MEANING = {
    ('壬', '丙'): '水火相衝 → 脾氣火爆、容易與人衝突',
    ('丙', '壬'): '水火相衝 → 脾氣火爆、容易與人衝突',
}
...
# 地支固定落宮（EP06）— 永遠不變
# 四正宮：子→坎1, 卯→震3, 午→離9, 酉→兌7（各一個地支）
# 四維宮：丑寅→艮8, 辰巳→巽4, 未申→坤2, 戌亥→乾6（各兩個地支）
DIZHI_GONG = {
    '子': 1,                          # 坎一宮（四正）
    '丑': 8, '寅': 8,                 # 艮八宮（四維）
    '卯': 3,                          # 震三宮（四正）
...
# 四正宮 vs 四維宮
SI_ZHENG_GONG = [1, 3, 9, 7]   # 坎震離兌
SI_WEI_GONG  = [8, 4, 2, 6]   # 艮巽坤乾

# 地支時間/月份/生肖對照（EP06）
DIZHI_INFO = {
    '子': {'time': '23:00-01:00', 'month': '農曆十一月', 'zodiac': '鼠'},
    '丑': {'time': '01:00-03:00', 'month': '農曆十二月', 'zodiac': '牛'},
    '寅': {'time': '03:00-05:00', 'month': '農曆正月',   'zodiac': '虎'},
    '卯': {'time': '05:00-07:00', 'month': '農曆二月',   'zodiac': '兔'},
    '辰': {'time': '07:00-09:00', 'month': '農曆三月',   'zodiac': '龍'},
    '巳': {'time': '09:00-11:00', 'month': '農曆四月',   'zodiac': '蛇'},
    '午': {'time': '11:00-13:00', 'month': '農曆五月',   'zodiac': '馬'},
    '未': {'time': '13:00-15:00', 'month': '農曆六月',   'zodiac': '羊'},
    '申': {'time': '15:00-17:00', 'month': '農曆七月',   'zodiac': '猴'},
    '酉': {'time': '17:00-19:00', 'month': '農曆八月',   'zodiac': '雞'},
    '戌': {'time': '19:00-21:00', 'month': '農曆九月',   'zodiac': '狗'},
    '亥': {'time': '21:00-23:00', 'month': '農曆十月',   'zodiac': '豬'},
}
...
# 天干相衝（4组）— 同阴阳 + 五行相剋 = 不稳定、逢衝必動（EP05）
# 戊己同屬土，性质稳定，无相衝
TIANGAN_XIANGCHONG = {
    '甲': '庚', '庚': '甲',
    '乙': '辛', '辛': '乙',
...
# 寄宮機制（EP06）— 轉盤特性
# 轉盤不用中五宮，但9天干+9星要排布，中五必然分配到一個
# 解決方法：中五宮的天干和天星寄放到坤二宮
# 結果：坤二宮會出現兩個天干+兩個天星
# ⚠️ 飛盤無此問題（飛盤九宮全用），此為轉盤特有
JIGONG_RULE = {
    'target_palace': 2,  # 寄到坤二宮
    'reason': '轉盤不用中五宮，中五的天干和天星寄放坤二',
    'impact': '分析時需兼顧寄宮的額外信息',
}
...
# ⚠️ 重要：用神有多面性！
# 工作穩定分析：開門=工作職位
# 找工作分析：開門=目前職位（不代表新工作！）
# 時干才代表求職的新工作
YONGSHEN_MULTI_FACET = {
    '開門': {
        'job_stability': '目前工作職位',
        'job_hunt': '目前工作職位（非新工作）',
        'investment': '交易',
    },
    '時干': {
        'love': '戀愛對象',
...
# 比和宮位對（EP04）— 同屬性五行的宮位互為比和
BIHE_PAIRS = [
    (3, 4, '木', '震三宮 + 巽四宮'),
    (6, 7, '金', '乾六宮 + 兌七宮'),
    (2, 8, '土', '坤二宮 + 艮八宮'),  # 師傅說坤艮還有另一種關係，待後續補充
]

# 雙用神原則（EP04）— 兩個用神可同時代表同一人/事物時，必須綜合分析
DUAL_YONGSHEN_SCENARIOS = {
    'love_male_query': {
        'desc': '男士測戀愛（真心與否）',
        'primary': {'self': '庚', 'partner': '乙'},
        'secondary': {'self': '日干', 'partner': '時干'},
    },
    'love_female_query': {
        'desc': '女士測戀愛（真心與否）',
        'primary': {'self': '乙', 'partner': '庚'},
        'secondary': {'self': '日干', 'partner': '時干'},
    },
}
...
YONGSHEN_CRIMINAL = {
    '疑犯': {'primary': '辛', 'meaning': '辛=犯錯/錯誤'},
    '警察': {'symbols': ['傷門', '白虎', '庚']},
    '檢控': {'symbols': ['杜門']},
    '法院': {'symbols': ['開門']},
    '監獄': {'辛': '天獄', '壬': '地牢', '癸': '天網'},
    '判刑': {'symbol': '庚', 'dual_role': '警察+判刑'},
}

# 定罪判斷規則（EP18）
CONVICTION_RULES = [
    {'id': 'R01', 'condition': '開門生辛', 'verdict': '無罪', 'severity': 0},
    {'id': 'R02', 'condition': '開門辛比和', 'verdict': '定罪(輕判/緩刑)', 'severity': 1},
    {'id': 'R03', 'condition': '開門剋/衝辛', 'verdict': '定罪(重判)', 'severity': 2},
    {'id': 'R04', 'condition': '庚+辛同宮', 'verdict': '定罪(重判)', 'severity': 2},
    {'id': 'R05', 'condition': '辛+辛/壬/癸', 'verdict': '定罪+監禁', 'severity': 2},
    {'id': 'R06', 'condition': '庚宮剋辛宮', 'verdict': '定罪', 'severity': 1},
    {'id': 'R07', 'condition': '辛落巽四宮', 'verdict': '傾向定罪', 'severity': 1},
]
...
# 特殊: 乙在坤二宮也入墓(代替甲)
TIANGAN_RUMU_SPECIAL = {'乙': [6, 2]}  # 乙有兩個入墓宮位
...
# 五行入墓規則（EP18）
WUXING_RUMU = {'土': 4, '水': 4, '木': 2, '火': 6, '金': 8}
